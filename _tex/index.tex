...
the observed correlation. Lower values than expected indicates than an
item is underfit to the Rasch model, while higher values indicate
overfit. The item-restscore function used in this simulation is from the
\texttt{iarm} package (Mueller and Santiago 2022) and outputs
Benjamini-Hochberg corrected \emph{p}-values (Benjamini and Hochberg
1995), which are used in determining whether the observed value is
statistically significant (using \emph{p} \textless{} .05 as critical
value) from the expected value for each item.
