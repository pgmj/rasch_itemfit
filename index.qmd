...
@fig-comp10 and @fig-comp2040 summarize the findings from the two different comparisons of item numbers.  Adding more items improves the detection rate substantially for both methods, particularly for smaller samples and the off-target items. It also results in a larger improvement for infit over item-restscore, especially for the n = 150 condition, but also the n = 250 condition for off-target items.
